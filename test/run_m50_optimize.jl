@testset "optimize m50" begin
    ...
    @testset "ep_nn train" begin
        ep_grad_opt = HydroModelTools.GradOptimizer(component=NamedTupleIOAdapter(ep_nn_flux), solve_alg=Adam(1e-2), adtype=Optimization.AutoZygote(), maxiters=100)
    ...
    @testset "ep_nn" begin
        q_grad_opt = HydroModelTools.GradOptimizer(component=NamedTupleIOAdapter(q_nn_flux), solve_alg=Adam(1e-2), adtype=Optimization.AutoZygote(), maxiters=100)
        ...
        q_opt_params, qnn_loss_df = q_grad_opt(
            [nn_input[HydroModels.get_input_names(q_nn_flux)]], [q_output],
