@testset "test compact with the DifferentialEquations.jl solvers (in Exphydro snow bucket)" begin
    ...
    @testset "test the ODEProblem and DiscreteProblem Solvers in single node" begin
        ode_result_1 = snow_ele(single_input, single_pas; initstates=init_states, timeidx=ts, solver=HydroModelTools.ODESolver())
        ode_result_2 = snow_ele(
            single_input, single_pas;
            initstates=init_states, timeidx=ts,
            solver=HydroModelTools.ODESolver(alg=BS3(), sensealg=BacksolveAdjoint(autojacvec=ZygoteVJP()), kwargs=Dict(:reltol => 1e-6, :abstol => 1e-6))
        )
        disc_result = snow_ele(single_input, single_pas; initstates=init_states, timeidx=ts, solver=HydroModelTools.DiscreteSolver())
    ...
    @testset "test the ODEProblem and DiscreteProblem Solvers in multiple node" begin
        ode_result_1 = snow_ele(
            ...
            ptyidx=1:10, styidx=1:10, timeidx=ts, solver=HydroModelTools.ODESolver()
        )
        ode_result_2 = snow_ele(
            node_input, node_pas;
        ...
        disc_result = snow_ele(node_input, node_pas;
            initstates=node_states, timeidx=ts, ptyidx=1:10, styidx=1:10, solver=HydroModelTools.DiscreteSolver()
        )
        @test size(ode_result_1) == size(ode_result_2) == size(disc_result)
    end
end
...
@testset "test compact with the DifferentialEquations.jl solvers (in Exphydro snow bucket)" begin
    ...
    @testset "test the ODEProblem and DiscreteProblem Solvers in single node" begin
        ode_result_1 = model(single_input, single_pas; initstates=init_states, config=(timeidx=ts, solver=HydroModelTools.ODESolver()))
        ode_result_2 = model(
            single_input, single_pas;
            initstates=init_states,
            config=(timeidx=ts, solver=HydroModelTools.ODESolver(alg=BS3(),
                sensealg=BacksolveAdjoint(autojacvec=ZygoteVJP()), kwargs=Dict(:reltol => 1e-6, :abstol => 1e-6)))
        )
        disc_result = model(single_input, single_pas; initstates=init_states, timeidx=ts, solver=HydroModelTools.DiscreteSolver())
        @test size(ode_result_1) == size(ode_result_2) == size(disc_result)
    end
    ...
    @testset "test the ODEProblem and DiscreteProblem Solvers in multiple node" begin
        ode_result_1 = model(
            ...
            config=(ptyidx=1:10, styidx=1:10, timeidx=ts, solver=HydroModelTools.ODESolver())
        )
        ode_result_2 = model(
            node_input, node_pas;
        ...
        disc_result = model(node_input, node_pas;
            initstates=node_initstates, config=(timeidx=ts, ptyidx=1:10, styidx=1:10, solver=HydroModelTools.DiscreteSolver())
        )
        @test size(ode_result_1) == size(ode_result_2) == size(disc_result)
    end
